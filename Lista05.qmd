---
title: "Lista05"
author: "Nathan Recco Neves_277213"
format: pdf
editor: source
---
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1.20 AS INCREASEDAREA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES / 3.0 AS DECREASEDAREA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50.00) / 2.0 AS ADJUSTEDFEE FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS MEANNEWFEE FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS SOMA, AVG(SALARY) AS MEDIA, MAX(SALARY) AS MAXIMO, MIN(SALARY) AS MINIMO FROM EMPLOYEE
```
